...
# Detect if running in distrobox - check multiple indicators
running_distrobox = (
    "DISTROBOX_HOST_HOME" in os.environ or 
    "CONTAINER_ID" in os.environ or
    os.path.exists("/run/.containerenv") or
    os.path.exists("/run/.toolboxenv") or
    os.path.exists(os.path.expanduser("~/.distrobox")) or
    os.path.exists(os.path.join(ws_dir, ".distrobox"))  # Check workspace marker
)

print(f"Running in distrobox: {running_distrobox}")
...
if running_distrobox:
    print("Copying ROS packages for VSCode intellisense...")
    # When in container, paths on host are accessed via DISTROBOX_HOST_HOME
    if "DISTROBOX_HOST_HOME" in os.environ:
        host_home = os.environ["DISTROBOX_HOST_HOME"]
        site_dir = os.path.join(host_home, ".vscode-paths/ros2-site-packages")
        dist_dir = os.path.join(host_home, ".vscode-paths/ros2-dist-packages")
        include_dir = os.path.join(host_home, ".vscode-paths/ros2-include")
        usr_include_dir = os.path.join(host_home, ".vscode-paths/usr-include")
...
if running_distrobox:
    config["terminal.integrated.profiles.linux"] = {
        "isaac_ws": {"path": f"{ws_dir}/scripts/distrobox"}
    }
else:
    config["terminal.integrated.profiles.linux"] = {
        "isaac_ws": {
...
config["terminal.integrated.defaultProfile.linux"] = "isaac_ws"
